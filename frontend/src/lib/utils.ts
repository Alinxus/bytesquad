...
/**
 * Format a minor unit amount (cents/kobo) to a human-readable currency string
 */
...
/**
 * Format a minor unit amount to a plain number string without currency symbol
 */
...
export function getStatusColors(status: string): { bg: string; text: string; border: string } {
  const statusMap: Record<string, { bg: string; text: string; border: string }> = {
    // Invoice statuses
    DRAFT: { bg: 'bg-surface-3', text: 'text-text-secondary', border: 'border-border' },
    ...
    EXPIRED: { bg: 'bg-warning/10', text: 'text-warning', border: 'border-warning/20' },
    // Withdrawal statuses
    ...
    // KYC statuses
    NOT_STARTED: { bg: 'bg-surface-3', text: 'text-text-secondary', border: 'border-border' },
  ...
  return statusMap[status] || { bg: 'bg-surface-3', text: 'text-text-secondary', border: 'border-border' }
}
